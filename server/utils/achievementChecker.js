...
async function getUserStats(userId) {
  try {
    const [
      ...
      prisma.session.findMany({ where: { hostId: userId }, select: { id: true, courseCode: true, location: true, time: true, date: true, status: true, members: true, tags: true, duration: true, hostAttended: true } }),
      prisma.sessionMember.findMany({ where: { userId } }),
      ...
      prisma.connection.count({ where: { OR: [{ fromUserId: userId }, { toUserId: userId }], status: 'accepted' } }),
    ...
    const uniqueKudosTags = await prisma.kudos.findMany({ where: { toUserId: userId }, select: { tag: true }, distinct: ['tag'] })
    const connectionRequestsSent = await prisma.connection.count({ where: { fromUserId: userId } })
    const connectionsAccepted = await prisma.connection.count({ where: { toUserId: userId, status: 'accepted' } })
    ...
    const weekendSessions = hostedSessions.filter(s => { const day = new Date(s.date).getDay(); return day === 0 || day === 6 }).length
    const librarySessions = hostedSessions.filter(s => s.location === 'Bluford Library').length
    const fullSessions = hostedSessions.filter(s => s.status === 'full').length
