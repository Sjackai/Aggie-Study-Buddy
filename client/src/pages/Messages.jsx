...
const fetchQR = async (sessionId) => {
  ...
  useEffect(() => {
    if (selectedThread) {
      const token = localStorage.getItem('token')
      directPollRef.current = setInterval(() => fetchDirectMessages(token, selectedThread.userId), 5000)
    }
  ...
  useEffect(() => {
    if (selectedGroupChat) {
      const token = localStorage.getItem('token')
      groupPollRef.current = setInterval(() => fetchGroupMessages(token, selectedGroupChat.id), 5000)
    }
  ...
  const fetchDirectMessages = async (token, userId) => {
    try {
      const res = await axios.get(`${API_URL}/api/messages/${userId}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const fetchGroupMessages = async (token, chatId) => {
    try {
      const res = await axios.get(`${API_URL}/api/groupchats/${chatId}/messages`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const openThread = async (thread) => {
    ...
    try {
      const res = await axios.get(`${API_URL}/api/messages/${thread.userId}`, { headers: { Authorization: `Bearer ${token}` } })
      setMessages(res.data)
      fetchAll(token)
    } catch (err) { console.error(err) }
  }
  ...
  const openRequest = async (request) => {
    ...
    setShowChat(true)
    try {
      const res = await axios.get(`${API_URL}/api/messages/${request.userId}`, { headers: { Authorization: `Bearer ${token}` } })
      setMessages(res.data)
    } catch (err) { console.error(err) }
  }
  ...
  const openGroupChat = async (chat) => {
    ...
    try {
      await axios.post(`${API_URL}/api/notifications/groupchat/${chat.id}/read`, {}, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSendDirect = async () => {
    ...
    setSending(true)
    try {
      const res = await axios.post(`${API_URL}/api/messages`, { receiverId: selectedThread.userId, text }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSendGroup = async () => {
    ...
    setSending(true)
    try {
      const res = await axios.post(`${API_URL}/api/groupchats/${selectedGroupChat.id}/messages`, { text }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleAcceptRequest = async (userId) => {
    const token = localStorage.getItem('token')
    try {
      await axios.post(`${API_URL}/api/messages/requests/${userId}/accept`, {}, { headers: { Authorization: `Bearer ${token}` } })
      showToast('Message request accepted! 💬')
      fetchAll(token)
      setSelectedRequest(null)
      setShowChat(false)
      setActiveTab('direct')
    } catch (err) { showToast('Failed to accept request', 'error') }
  }

  const handleDeclineRequest = async (userId) => {
    const token = localStorage.getItem('token')
    try {
      await axios.post(`${API_URL}/api/messages/requests/${userId}/decline`, {}, { headers: { Authorization: `Bearer ${token}` } })
      showToast('Request declined')
      fetchAll(token)
      setSelectedRequest(null)
      setShowChat(false)
    } catch (err) { showToast('Failed to decline request', 'error') }
  }
  ...
  const handleLeaveGroupChat = async (chatId) => {
    ...
    try {
      await axios.post(`${API_URL}/api/sessions/${chat?.sessionId}/leave`, {}, { headers: { Authorization: `Bearer ${token}` } })
      setSelectedGroupChat(null)
      setShowChat(false)
      setLeaveGroupConfirm(null)
      showToast('You have left the session and group chat')
      fetchAll(token)
    } catch (err) { showToast(err.response?.data?.error || 'Failed to leave', 'error') }
  }

  const handleApproveRejoin = async (sessionId, userId) => {
    const token = localStorage.getItem('token')
    try {
      await axios.post(`${API_URL}/api/sessions/${sessionId}/approve/${userId}`, {}, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleDeclineRejoin = async (sessionId, userId) => {
    const token = localStorage.getItem('token')
    try {
      await axios.post(`${API_URL}/api/sessions/${sessionId}/decline/${userId}`, {}, { headers: { Authorization: `Bearer ${token}` } })
      showToast('Rejoin request declined')
      fetchAll(token)
    } catch (err) { showToast('Failed to decline', 'error') }
  }
  ...
  const handleSearchUsers = async (q) => {
    ...
    setSearching(true)
    try {
      const res = await axios.get(`${API_URL}/api/users/search?q=${q}`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleStartNewMessage = async (user) => {
    ...
    try {
      await axios.post(`${API_URL}/api/messages`, { receiverId: user.id, text: `Hey ${user.name?.split(' ')[0]}! 👋` }, { headers: { Authorization: `Bearer ${token}` } })
      fetchAll(token)
      openThread({ userId: user.id, name: user.name, major: user.major, unread: 0, lastMessage: null })
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col">
      <nav className="bg-ncat-blue px-6 py-4 flex justify-between items-center flex-shrink-0 shadow-md">
      ...
      {rejoinRequests.map(req => (
        <div key={req.userId} className="fixed top-20 right-4 z-50 bg-white rounded-2xl shadow-xl border border-gray-100 p-4 w-72">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleDeclineRejoin(req.sessionId, req.userId)}
              className="flex-1 border border-gray-200 text-gray-600 text-xs font-bold py-2 rounded-xl hover:bg-gray-50 transition">
              Decline
            </button>
            <button onClick={() => handleApproveRejoin(req.sessionId, req.userId)}
        ...
        <div className={`${showChat ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-80 bg-white border-r border-gray-100 flex-shrink-0`}>
          <div className="p-4 border-b border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <p className="font-bold text-gray-800 text-lg">Messages</p>
              <button onClick={() => setShowNewMessage(true)}
                className="w-9 h-9 bg-ncat-blue rounded-full flex items-center justify-center text-white hover:opacity-90 transition shadow-sm"
                title="New Message">
                ✏️
              </button>
            </div>
            <div className="flex rounded-xl overflow-hidden border border-gray-200">
              <button onClick={() => setActiveTab('direct')}
                className={`flex-1 py-2 text-xs font-semibold transition ${activeTab === 'direct' ? 'bg-ncat-blue text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                💬 Direct
              ...
              <button onClick={() => setActiveTab('requests')}
                className={`flex-1 py-2 text-xs font-semibold transition border-l border-r border-gray-200 ${activeTab === 'requests' ? 'bg-ncat-blue text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                📬 Requests
                {requests.length > 0 && (
              ...
              <button onClick={() => setActiveTab('sessions')}
                className={`flex-1 py-2 text-xs font-semibold transition ${activeTab === 'sessions' ? 'bg-ncat-blue text-white' : 'text-gray-500 hover:bg-gray-50'}`}>
                👥 Groups
                {unreadGroupCount > 0 && (
                  ...
                  <button key={thread.userId} onClick={() => openThread(thread)}
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 ${getColor(thread.name)} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 overflow-hidden`}>
                        {thread.avatar ? <img src={thread.avatar} alt={thread.name} className="w-full h-full object-cover" /> : getInitials(thread.name)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-0.5">
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                            ...
                            {thread.unread > 0 && (
                              <span className="bg-ncat-blue text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">{thread.unread}</span>
                            )}
                          </div>
                        </div>
                  ...
                  <button key={request.userId} onClick={() => openRequest(request)}
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 ${getColor(request.name)} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0 overflow-hidden`}>
                        {request.avatar ? <img src={request.avatar} alt={request.name} className="w-full h-full object-cover" /> : getInitials(request.name)}
                      </div>
                      <div className="flex-1 min-w-0">
                  ...
                  <button key={chat.id} onClick={() => openGroupChat(chat)}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative flex-shrink-0">
                        <div className="w-12 h-12 bg-gradient-to-br from-ncat-blue to-blue-700 rounded-full flex items-center justify-center text-white font-bold text-lg shadow-sm">👥</div>
                        {chat.hasUnread && (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-0.5">
                          ...
                          <span className="text-xs text-gray-400 flex-shrink-0 ml-2">{chat.members?.filter(m => !m.leftAt).length} members</span>
                        </div>
                        {chat.messages?.[0] && (
                          <p className={`text-xs truncate ${chat.hasUnread ? 'text-gray-800 font-semibold' : 'text-gray-400'}`}>
                            {chat.messages[0].isSystem ? chat.messages[0].text : `${chat.messages[0].sender?.name?.split(' ')[0]}: ${chat.messages[0].text}`}
                          </p>
                        )}
                      </div>
                    </div>
          ...
          {!selectedThread && !selectedGroupChat && !selectedRequest ? (
            <div className="flex-1 flex items-center justify-center bg-gray-50">
              <div className="text-center">
                ...
                <button onClick={() => setShowNewMessage(true)} className="mt-4 bg-ncat-blue text-white font-bold px-5 py-2.5 rounded-xl hover:opacity-90 transition text-sm shadow-md">
                  ✏️ New Message
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="bg-white px-6 py-4 border-b border-gray-100 flex items-center gap-3 flex-shrink-0 shadow-sm">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="bg-white px-6 py-4 border-b border-gray-100 flex items-center gap-3 flex-shrink-0 shadow-sm">
                <div className={`w-10 h-10 ${getColor(selectedThread.name)} rounded-full flex items-center justify-center text-white font-bold text-sm cursor-pointer overflow-hidden`}
                  ...
                  {selectedThread.avatar ? <img src={selectedThread.avatar} alt={selectedThread.name} className="w-full h-full object-cover" /> : getInitials(selectedThread.name)}
                </div>
                <div className="flex-1">
                  <p className="font-bold text-gray-800 cursor-pointer hover:text-ncat-blue transition" onClick={() => navigate(`/profile/${selectedThread.userId}`)}>
                    {selectedThread.name}
                  </p>
                    ...
                    <div key={msg.id} className={`flex ${msg.senderId === currentUser?.id ? 'justify-end' : 'justify-start'}`}>
                      {msg.senderId !== currentUser?.id && (
                        <div className={`w-8 h-8 ${getColor(msg.sender?.name)} rounded-full flex items-center justify-center text-white font-bold text-xs mr-2 flex-shrink-0 self-end overflow-hidden`}>
                          {msg.sender?.avatar ? <img src={msg.sender.avatar} alt={msg.sender.name} className="w-full h-full object-cover" /> : getInitials(msg.sender?.name)}
                        </div>
                      )}
              ...
              <div className="p-4 border-t border-gray-100 bg-white flex gap-2">
                ...
                <button onClick={handleSendDirect} disabled={sending || !newMessage.trim()}
                  className="bg-ncat-blue text-white font-bold w-12 h-12 rounded-2xl hover:opacity-90 transition disabled:opacity-40 flex items-center justify-center text-lg flex-shrink-0">
                  ➤
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="bg-white border-b border-gray-100 flex-shrink-0 shadow-sm">
                <div className="px-6 py-3 flex items-center gap-3">
                  <div className="w-10 h-10 bg-gradient-to-br from-ncat-blue to-blue-700 rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 shadow-sm">👥</div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-800 truncate">{selectedGroupChat.name}</p>
                ...
                <div className="px-6 pb-3 flex items-center gap-2 overflow-x-auto scrollbar-hide">
                  {selectedGroupChat.members?.filter(m => !m.leftAt).map((member, i) => (
                    <div key={i} className="flex flex-col items-center gap-1 flex-shrink-0 cursor-pointer group"
                      ...
                      <div className={`w-8 h-8 ${getColor(member.user?.name)} rounded-full flex items-center justify-center text-white font-bold text-xs overflow-hidden border-2 border-white shadow-sm group-hover:border-ncat-blue transition`}>
                        {member.user?.avatar ? <img src={member.user.avatar} alt="" className="w-full h-full object-cover" /> : getInitials(member.user?.name)}
                      </div>
                      <span className="text-xs text-gray-400 max-w-12 truncate group-hover:text-ncat-blue transition">{member.user?.name?.split(' ')[0]}</span>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  groupMessages.map(msg => (
                    msg.isSystem ? (
                      <div key={msg.id} className="flex justify-center my-2">
                        <p className="text-xs text-gray-400 bg-gray-100 px-3 py-1.5 rounded-full">{msg.text}</p>
                      </div>
                    ) : (
                      <div key={msg.id} className={`flex ${msg.senderId === currentUser?.id ? 'justify-end' : 'justify-start'}`}>
                        {msg.senderId !== currentUser?.id && (
                          <div className={`w-8 h-8 ${getColor(msg.sender?.name)} rounded-full flex items-center justify-center text-white font-bold text-xs mr-2 flex-shrink-0 self-end cursor-pointer overflow-hidden`}
                            onClick={() => navigate(`/profile/${msg.senderId}`)}>
                            {msg.sender?.avatar ? <img src={msg.sender.avatar} alt="" className="w-full h-full object-cover" /> : getInitials(msg.sender?.name)}
                          </div>
                        )}
                        <div className="max-w-xs md:max-w-md">
                          {msg.senderId !== currentUser?.id && (
                            <p className="text-xs text-gray-400 mb-1 ml-1 font-medium">{msg.sender?.name?.split(' ')[0]}</p>
                          )}
                    ...
                    <div key={member.userId} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {member.checkedInAt && <p className="text-xs text-gray-400">at {formatTime(member.checkedInAt)}</p>}
                        </div>
                      </div>
              ...
              {searching ? (
                ...
                searchResults.map(user => (
                  <button key={user.id} onClick={() => handleStartNewMessage(user)}
                    ...
                    <div className={`w-10 h-10 ${getColor(user.name)} rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 overflow-hidden`}>
