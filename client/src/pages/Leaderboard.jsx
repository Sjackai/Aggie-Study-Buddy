          ...
          {tabs.map(tab => (
            <button
              ...
              className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-bold transition shadow-sm ${
                activeTab === tab.id ? 'bg-ncat-blue text-white shadow-md' : 'bg-white text-gray-600 hover:shadow-md'
              }`}
            >
              {tab.label}
            </button>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="divide-y divide-gray-50">
              {data.map((entry, i) => (
                <div key={entry.userId} className={`flex items-center gap-3 px-6 py-4 ${entry.userId === currentUser?.id ? 'bg-blue-50' : 'hover:bg-gray-50'} transition`}>
